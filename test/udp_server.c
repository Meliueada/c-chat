...
static void _login_ucnode(ucnode_t head, int sd, struct sockaddr_in* pa, struct umsg* msg) {
    ...
    while(head->next){
        head = head->next;
        IF_CHECK(sendto(sd, msg, , (struct sockaddr*)&head->addr, sizeof(struct sockaddr_in)));
    }
}
...
static void _quit_ucnode(ucnode_t head, int sd, struct sockaddr_in* pa, struct umsg* msg) {
    ...
    while(head->next) {
            ...
            IF_CHECK(sendto(sd, msg, , (struct sockaddr*)&head->next->addr, sizeof(struct sockaddr_in)));
            head = head->next;
        }
        else { //删除这个退出的用户
...
int main(int argc, char* argv[]) {
    ...
    for(;;){
        ...
        IF_CHECK(recvfrom(sd, &msg, , (struct sockaddr*)&addr, &alen));
        msg.name[_INT_NAME-] = msg.text[_INT_TEXT-] = '\0';
        fprintf(stdout, "msg is [%s:%d] => [%c:%s:%s]\n", inet_ntoa(addr.sin_addr),
                    ntohs(addr.sin_port), msg.type, msg.name, msg.text);
        // 开始判断处理
        switch(msg.type) {
        ':_login_ucnode(head, sd, &addr, &msg);break;
        ':_broadcast_ucnode(head, sd, &addr, &msg);break;
        ':_quit_ucnode(head, sd, &addr, &msg);break;
